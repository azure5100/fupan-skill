...
- **v2.9.18（2026-09-24）**：**两处长期遗留 bug 修复（报告自检发现）**——①**`复盘总结` 家数口径错**：总结行用 `_zt_n = len(ztpool)`（pool 实时家数），而 2.2 面板用 `int(_zt or _zt_n)`（vision 优先），两者不一致时报告自相矛盾（9/24 实测：pool 51 只 vs vision 52 家 → 总结误显"涨停 51 家"，面板显 52）。修法：新增 `_zt_v = int(_zt or _zt_n)` / `_dt_v = int(_dt or _dt_n)`，2.2 面板 / 复盘总结 / 日志 统一引用（lines 293-296、308、314、660-666、684）。②**成交额 xychart mermaid 语法坏**：`f'    x-axis ["{"", "".join(_names)}"]'` 花括号内含**逗号** → Python 解析成**元组**，渲染出 `x-axis ["('', '新华传媒泰慕士平潭发展...')"]`——板块名无分隔符粘连、bar 值连成 `19.23.42.32.1...`，图表实际不可用（9/22、9/23 报告同款）。修法：提到 f-string 外拼接 —— `_xaxis = ', '.join(f'"{n}"' for n in _names)` / `_bars = ', '.join(_vals)` → **f-string 花括号里出现逗号即元组陷阱**，生成 mermaid/JSON 时一律先拼好字符串再插入。修后重跑 fupan_run：`复盘总结` 涨停 52 与面板一致，xychart 输出标准 `x-axis ["A", "B", ...]` / `bar [17.6, 3.6, ...]`
- **v2.9.17（2026-09-21）**：**§4.1 板块重分类第五次复现（15→14，细分→产业链合并口径）**——18:12 采集 15 细分板块（医药医疗 21/房地产 13/大消费 10/农林牧渔 5/AI硬件 4/AI应用 4/PCB 4/传媒 3/半导体 3/外贸出口 3/机器人 3/大金融 2/光通信 2/其他 19/公告 7 = 103）生成报告 → 18:13 探测页面一致 → 18:2x 用户反馈"四、市场题材全景图 数据不对" → 重采确认站点已合并为 **14 产业链板块**（医药 24/地产产业链 12/大消费 12/算力·半导体产业链 8/其他概念 8/有色金属·小金属 7/传媒·AI应用 7/AI终端·面板 5/机器人·汽车产业链 5/大农业 5/军工·航天 3/PCB产业链 3/MLCC·电容 2/大金融 2 = 103 股，总数不变）。处理：fupan_run 全流程重跑（注意：单独跑 gen_report 会因 pool JSON 已被 verify 清理而生成空报告——必须走 fupan_run 重新 pull_data）→ verify 35/35 → 4.1 与页面 ground truth 完全一致。**规律确认**：采集后 30-60 分钟内站点可能合并分组（第五次复现：8/10、8/13、8/17、8/28、9/21），用户反馈不符时**必须重采重生成**
